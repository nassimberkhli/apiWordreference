...
def fetch_page(word, dict_code):
    try:
        ...
        print(f"[fetch_page] status={response.status_code} final_url={response.url}")
        response.raise_for_status()
        return response.text
    except requests.RequestException as e:
        print(f"[fetch_page] Error fetching page: {e}")
        return None
...
def parse_translation(html_content, specific_meanings):
    ...
    print(f"[parse_translation] rows_found={len(results)}")
    ...
    for row in results:
        ...
        if row.find(class_="FrWrd"):
            ...
            if (
                ...
                and (specific_meanings == [] or translation_number in specific_meanings)
            ):
                translations[translation_number] = translation

        elif (
            row.find(class_="ToWrd")
            or row.find(class_="FrEx")
            or row.find(class_="ToEx")
        ):
            if translation and (
    ...
    merged = merge_translation_entries(translations)

    print(
        f"[parse_translation] raw_translations={len(translations)} merged={len(merged)} audio={len(audio_links)}"
    )
    return merged, audio_links
...
def extract_and_clean_meanings(raw_text):
    ...
    parts = re.split(
        r"[,:;]|(?<=\w)\s+(?=s[\'e]|se|être|devenir|adhérer|venir|aller|tomber|avoir|faire|prendre)",
        raw_text,
    )
    ...
    for part in parts:
        ...
        if re.search(
            r"\b(v|adj|n|adv|prep|loc|vi|vt|nm|nf|pron|intj|conj|pl|ind)\b",
            part,
            re.IGNORECASE,
        ):
            continue
...
def clean_text(text, type_=""):
    ...
    if type_ == "meanings":
        ...
        text = text.replace("+", "")
        text = re.sub(
            r"\b(v|adj|n|adv|prep|loc|vi|vt|nm|nf|pron|intj|conj|pl|ind)\b", "", text
        )
